...
public:
	vector<vector<int>> floodFill(vector<vector<int>>& image, int sr, int sc, int newColor)
	{
		...
		dfs(image, flag, sr, sc, row, col, color, newColor);
		return image;
	}
	int nextposition[4][2] = { {0,1},{1,0},{0,-1},{-1,0} };
	void dfs(vector<vector<int>>& image, vector<vector<int>>& flag, int sr, int sc, int row, int col, int color, int newColor)
	{
		//处理当前逻辑
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			if (flag[newsr][newsc] == 0 && image[newsr][newsc] == color)
			{
				dfs(image, flag, newsr, newsc, row, col, color, newColor);
			}
		}
	}
};
